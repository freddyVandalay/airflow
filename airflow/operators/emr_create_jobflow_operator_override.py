...
class EmrCreateJobFlowOperatorOverride(EmrCreateJobFlowOperator):
    """
    This operator is part of data managements solution to live testing and development of dags containing spark scripts

    The operator recognises the deployment environment and if local:
        * uploads the local airflow repository and bootstrap.sh to the users s3 bucket under 'grp-ds-users'
          (firstname.lastname)
        * overrides the emr template and specifies development in the cluster name
        * overrides the bootstrap location to point to users local file in s3 to which it was uploaded in a the
          previous step
